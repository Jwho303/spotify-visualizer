class SpotifyVisualizer {
    ...
    setupMediaControls() {
        document.getElementById('play-pause-btn').addEventListener('click', async () => {
            try {
                const trackData = await this.spotifyAuth.getCurrentTrack();
                
                if (trackData.playing) {
                    await this.spotifyAuth.pause();
                    document.getElementById('play-pause-btn').textContent = '▶️';
                } else {
                    await this.spotifyAuth.play();
                    document.getElementById('play-pause-btn').textContent = '⏸️';
                }
            } catch (error) {
        ...
        document.getElementById('prev-btn').addEventListener('click', async () => {
            try {
                await this.spotifyAuth.previous();
    ...
    async startTrackPolling() {
        const pollTrack = async () => {
            try {
                const data = await this.spotifyAuth.getCurrentTrack();
    ...
    transitionBackground(backgroundColors) {
        if (!backgroundColors || backgroundColors.length < 2) return;
    ...
    startAnimation() {
        const animate = () => {
            ...
            if (this.audioEnabled && this.audioAnalyzer.isInitialized) {
